...
function removeFirstOccurrences(str, value) {
  const indValue = str.indexOf(value);
  return str.slice(0, indValue) + str.slice(indValue + value.length);
}
...
function removeLastOccurrences(str, value) {
  const indValue = str.lastIndexOf(value);
  return str.slice(0, indValue) + str.slice(indValue + value.length);
}
...
function formatTime(minutes, seconds) {
  const leftTime = minutes.toString().padStart(2, '0');
  const rightTime = seconds.toString().padStart(2, '0');
  return leftTime.concat(':', rightTime);
}
